    ...
    for dataset in config.datasets:
        ...
        for experiment_folder in config.experiment_folders:
            if experiment_folder == 'Reduced_Resolution':
                experiment_folder = os.path.join(experiment_folder, '60')
        ...
        for i, path in enumerate(ds_paths):
            ...
            save_root = os.path.join(config.save_root, dataset, name, experiment_type, '60')

            if not os.path.exists(os.path.join(save_assessment, '60')):
                os.makedirs(os.path.join(save_assessment, '60'))

            if experiment_type == 'RR':
                if not os.path.exists(os.path.join(save_assessment, '60', name + '_RR.csv')):
                    with open(os.path.join(save_assessment, '60', name + '_RR.csv'), 'w', encoding='UTF8', newline='') as f:
                        writer = csv.DictWriter(f, fieldnames=fieldnames_rr)
                        writer.writeheader()
                    f.close()

            else:
                if not os.path.exists(os.path.join(save_assessment, '60', name + '_FR.csv')):
                    with open(os.path.join(save_assessment, '60', name + '_FR.csv'), 'w', encoding='UTF8', newline='') as f:
            ...
            exp_info['ms_lr'] = bands_60
            exp_info['ms'] = it.ideal_interpolator(bands_60, ratio)
            exp_info['dataset'] = dataset
            exp_info['sensor'] = 'S2-60'
            exp_info['name'] = name
                ...
                if algorithm in pansharpening_algorithm_dict.keys():
                    ...
                    for pan_generation in config.pan_generation:
                        if pan_generation == 'selection':
                            ...
                            pan, selected_bands = selection(bands_10, bands_60, ratio, 'S2-10')
                            exp_input.pan = pan
                            ...
                            gen = 'SYNTH-'
                            print('Running algorithm: ' + gen + temp_name_algorithm)
                            pan, selected_bands = synthesize(bands_10, bands_60, ratio, 'S2-10')
                            exp_input.pan = pan
                            exp_input.pan_generator = pan_generation
                            exp_input.sensor_pan = ['S2-10-PAN'] * bands_60.shape[1]
                        ...
                        with torch.no_grad():
                            if experiment_type == 'RR':
                                ...
                                with open(os.path.join(save_assessment, '60', name + '_RR.csv'), 'a',
                                          encoding='UTF8', newline='') as f:
                                    writer = csv.DictWriter(f, fieldnames=fieldnames_rr)
                                    writer.writerow(metrics_values_rr_dict)
                            else:
                                metrics_values_fr = list(
                                    evaluation_fr(fused, torch.clone(bands_10), torch.clone(bands_60),
                                ...
                                with open(os.path.join(save_assessment, '60', name + '_FR.csv'), 'a',
                                          encoding='UTF8', newline='') as f:
                                    writer = csv.DictWriter(f, fieldnames=fieldnames_fr)
                        ...
                        if config.save_results:
                            ...
                            ut.save_mat(np.round(
                                ...
                                np.uint16), os.path.join(save_root, algorithm + '.mat'), 'S2_60')
                    ...
                    with torch.no_grad():
                        if experiment_type == 'RR':
                            ...
                            print(metrics_values_rr_dict)
                            metrics_rr.append(metrics_values_rr_dict)
                            with open(os.path.join(save_assessment, '60', name + '_RR.csv'), 'a', encoding='UTF8', newline='') as f:
                                writer = csv.DictWriter(f, fieldnames=fieldnames_rr)
                                writer.writerow(metrics_values_rr_dict)
                        else:
                            metrics_values_fr = list(evaluation_fr(fused, torch.clone(bands_10), torch.clone(bands_60), ratio=exp_info['ratio'], sensor=exp_info['sensor']))
                            metrics_values_fr.insert(0, algorithm)
                            metrics_values_fr.append(elapsed_time)
                            metrics_values_fr_dict = dict(zip(fieldnames_fr, metrics_values_fr))
                            print(metrics_values_fr_dict)
                            metrics_fr.append(metrics_values_fr_dict)
                            with open(os.path.join(save_assessment, '60', name + '_FR.csv'), 'a', encoding='UTF8', newline='') as f:
                                writer = csv.DictWriter(f, fieldnames=fieldnames_fr)
                                writer.writerow(metrics_values_fr_dict)
                    if config.save_results:
                        if not os.path.exists(save_root):
                            os.makedirs(save_root)
                        ut.save_mat(np.round(np.clip(np.squeeze(fused.permute(0, 2, 3, 1).numpy(), axis=0), 0, 2 ** 16 - 1)).astype(np.uint16), os.path.join(save_root, algorithm + '.mat'), 'S2_60')
