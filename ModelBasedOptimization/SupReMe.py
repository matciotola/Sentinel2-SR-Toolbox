import torch
from .tools import (normalize_data, denormalize_data, generate_stack, generate_mean_stack, conv_cm, compute_weights,
                    create_subsampling, create_conv_kernel, create_conv_kernel_subspace, regularization, conv2im, conv2mat)
...
def SupReMe(ordered_dict):
    ...
    bands_10_normalized, m_high = normalize_data(bands_10)
    bands_20_normalized, m_intermediate_lr = normalize_data(bands_20)
    bands_60_normalized, m_low_lr = normalize_data(bands_60)
    ...
    _, _, nl, nc = bands_10_normalized.shape
    n = nl * nc
    ...
    bands_20_upsampled = imresize(bands_20_normalized, 2)
    bands_60_upsampled = imresize(bands_60_normalized, 6)

    y1im = torch.cat(generate_stack(bands_10, bands_20_upsampled, bands_60_upsampled), dim=1)
