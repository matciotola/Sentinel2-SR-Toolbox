from .tools import generate_stack, generate_mean_stack, normalize_data, denormalize_data, \
...
def S2Sharp(ordered_dict):
    ...
    q = torch.tensor([1, 1.5, 4, 8, 15, 15, 20], dtype=bands_10.dtype, device=bands_10.device)
    ...
    bs, _, nl, nc = bands_10.shape
    n = nl * nc
    y_im = generate_stack(bands_10, bands_20, bands_60)
    nb = len(y_im)

    bands_10_norm, m_high = normalize_data(bands_10)
    bands_20_norm, m_intermediate_lr = normalize_data(bands_20)
    bands_60_norm, m_low_lr = normalize_data(bands_60)
    y_im_2 = generate_stack(bands_10_norm, bands_20_norm, bands_60_norm)
