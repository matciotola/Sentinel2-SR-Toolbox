...
def gen_mtf_pan(ratio, sensor, kernel_size=41):
    ...
    if sensor == 'S2-SEL-0':
        GNyq = np.asarray([0.275])
    elif sensor == 'S2-SEL-1':
        GNyq = np.asarray([0.28])
    elif sensor == 'S2-SEL-2':
        GNyq = np.asarray([0.25])
    elif sensor == 'S2-SEL-3':
        GNyq = np.asarray([0.24])
    elif sensor == 'S2-10-PAN':
        GNyq = np.asarray([0.26125])
    else:
