...
def SSSS(ordered_dict):
    ...
    y_im_list = generate_stack(bands_high, bands_intermediate_lr, bands_low_lr)

    y_im = torch.zeros([1, len(y_im_list), bands_high.shape[-2], bands_high.shape[-1]], dtype=bands_high.dtype,
    ...
    fused = torch.zeros([bs, nb, c, d], dtype=bands_high.dtype, device=bands_high.device)

    for i in range(a):
        row_start = row_start + (nr - 2 * limsub) * i - 1
        row_end = row_start + nr * (i + 1)
        fused[:, :, row_start:row_end, :] = ssss_algorithm(y_im[:, :, row_start:row_end, :], rv, dx, dy, sdf,
                                                           lambda_opt, mu)
...
def ssss_algorithm(y_im, rv, dx, dy, sdf, lambda_opt, mu):
    ...
    fbmc = torch.conj(fbm)
    bty = torch.flatten(
        torch.real(torch.fft.ifft2(torch.fft.fft2(torch.reshape(y, [bs, y.shape[1], nc, nr]).transpose(2, 3)) * fbmc)),
        start_dim=2)
    ...
    for i in range(num_admm_iter):
        nu = (u @ z) - torch.reshape(d, [bs, nb, n]).numpy()
        aux = bty + mu * nu
        for b in range(nb):
            inp = aux[:, b, :].flatten()
            r = rv[b]
        ...
        z = (((torch.reshape((v + d), [nb, n]).transpose(-2, -1) @ u) + torch.sum(v_ij + d_ij,
                                                                                  dim=2)) @ utu_plus_i_inv).transpose(
            -2, -1)
        ...
        if i == epochs:
            ...
            for b in range(nb):
                ...
                bzt = torch.flatten(torch.real(torch.fft.ifft2(torch.fft.fft2(input_1 * input_2))), start_dim=2)
